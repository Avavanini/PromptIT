...
function TreeItem({node, activeFile, onFileSelect, depth = 0 }){
    ...
    if(node.isDir){
        return (
            <div>
                <div className="flex items-center gap-2 py-1 px-2 text-xs text-[#7582B8] select-none" 
                style={{paddingLeft: `${depth * 12 + 8}px`}}>
                    <FolderOpenIcon size={14} className='text-[#FFFFFF] opacity-60'/>
                    <span>{node.name}</span>
                </div>
    ...
    return (
        ...
        className={`w-full flex items-center gap-2 py-1.5 px-2 text-xs transition-colors rounded-md cursor-pointer ${isActive ? "bg-[#1C2135] text-[#FF9E5E] font-medium" : "text-[#7582B8] hover:bg-[#1C2135] hover:text-[#FFFFFF]"}`}
        style={{paddingLeft: `${depth * 12 + 8}px`}}>
...
const FileExplorer = ({files, activeFile, onFileSelect }) => {
  ...
  return (
    <div className="py-2 overflow-y-auto hide-scrollbar">
        <p className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-[#7582B8]">Files</p>
            {tree.map((node)=>(
